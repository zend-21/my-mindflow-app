...
const getStemElement = (stem) => {
    return STEM_ELEMENT_MAP[stem] || 'Wood';
};
...
const getBranchElement = (branch) => {
    return BRANCH_ELEMENT_MAP[branch] || 'Earth';
};
...
const calculateElementInteraction = (userElement, todayElement) => {
    ...
    if (ELEMENT_GENERATING[userElement] === todayElement) {
        return 15;
    }

    // 상대가 나를 생해주는 경우 (인수: +20) - 가장 좋음
    if (ELEMENT_GENERATING[todayElement] === userElement) {
        return 20;
    }
...
const calculateSolarTerm = (date) => {
    ...
    return {
        ...
        index: SOLAR_TERMS.indexOf(currentTerm),
...
const getMonthElement = (month) => {
    return MONTH_ELEMENTS[month] || 'Earth';
};
...
const selectHoroscopeFortune = async (zodiacSign, date) => {
    ...
    const zodiacCode = ZODIAC_SIGN_MAP[zodiacSign] || 'AR';
...
const calculateCategoryScore = (userDayStem, todayPillar, categoryIndex) => {
    ...
    // 5️⃣ 카테고리별 가중치 조정
    const weights = CATEGORY_WEIGHTS[categoryIndex] || CATEGORY_WEIGHTS[0];
